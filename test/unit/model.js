...
describe("Model", function () {
  ...
  describe("#get", function () {
    ...
    it("uses its mapper to lazy-load any FK collections", function (done) {
      model.store.fka = ['model'];
      model.get('fka', function (err, instance) {
        if (err) throw err;
        assert.equal(instance[0].id, 'yes');
        done();
      }); 
    });
  });
